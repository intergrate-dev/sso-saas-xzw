...
public class HttpClient implements java.io.Serializable {
	...
	public Response get(String url, String token) throws HttpclientException {
	...
	/**
	 * 最简单的直接通过HTTPClient去get一个URL URL内容必须经过URLEncoding以保证安全
	 * 
	 * @param url
	 * @return
	 * @throws HttpclientException
	 */
	public Response get(String url) throws HttpclientException {
		log("Request:");
		log("GET:" + url);
		GetMethod getmethod = new GetMethod(url);
		return httpRequest(getmethod, false, null);
	...
	public Response get(String url, PostParameter[] params, Paging paging,
			String token) throws HttpclientException {
		if (null != paging) {
	...
	public Response post(String url, PostParameter[] params,
			Boolean WithTokenHeader, String token) throws HttpclientException {
		log("Request:");
		log("POST" + url);
	...
	public Response multPartURL(String url, PostParameter[] params,
			ImageItem item, String token) throws HttpclientException {
		PostMethod postMethod = new PostMethod(url);
		try {
			Part[] parts = null;
			if (params == null) {
			...
			throw new HttpclientException(ex.getMessage(), ex, -1);
		}
	}
	...
	public Response httpRequest(HttpMethod method, Boolean WithTokenHeader,
		...
		try {
			...
			if (responseCode != OK) {
				try {
					throw new HttpclientException(getCause(responseCode),
			...
			throw new HttpclientException(ioe.getMessage(), ioe, responseCode);
		} finally {
