...
public class WechatClient extends OauthClient{
	...
	public String getProvider() {
		return OauthProviders.WECHAT.getValue();
	}

	@Override
	public String getProviderName() {
		return OauthProviders.WECHAT.getName();
	}
	...
	public OauthAccessToken exchangeAccessToken(String authcCode) throws OauthErrorMsg {
		...
		accessToken = new OauthAccessToken(token, OauthProviders.WECHAT.getValue());
	...
	public RemoteUser getRemoteUser(ServletRequest request) {
		...
		try {
			...
			remoteUser.setTokenExpiresTime(DateUtils.addMilliseconds(new Date(), WechatClient.EXPIRE));
		} catch (Exception e) {
			e.printStackTrace();
		}
