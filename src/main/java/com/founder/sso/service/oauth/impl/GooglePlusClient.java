...
public class GooglePlusClient extends OauthClient{
	...
	public String getProvider() {
		return OauthProviders.GOOGLEPLUS.getValue();
	}

	@Override
	public String getProviderName() {
		return OauthProviders.GOOGLEPLUS.getName();
	}
	...
	public OauthAccessToken exchangeAccessToken(String authcCode) throws OauthErrorMsg {
		...
		accessToken = new OauthAccessToken(token, OauthProviders.GOOGLEPLUS.getValue());
	...
	public RemoteUser getRemoteUser(ServletRequest request) {
		...
		try {
			...
			remoteUser.setTokenExpiresTime(DateUtils.addMilliseconds(new Date(), GooglePlusClient.EXPIRE));
		} catch (Exception e) {
			e.printStackTrace();
		}
