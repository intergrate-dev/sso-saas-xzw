				...
				jQuery.validator.addMethod("soleCode", function(value, element) {
					...
					if(value!=""){
						$.ajax({ 
			    			async:false,
			    			type: "post", 
			    			url: "${ctx}/admin/subsystem/isCodeLegal?code=" + value + "&systemId=${subsystem.id}&random=" + new Date().getTime() , 
...
// 				}, "系统入口不是合法的url");	
	...
	<body>
		...
		<ul class="breadcrumb">
		  <li><a href="${ctx }/admin/subsystem/">子系统管理</a> <span class="divider">/</span></li>
...
<!-- 			     		<font style="font-size: 0.9em;color: red;">系统代码是子系统同步登录时的凭证，如需修改，请与相关子系统开发人员联系。</font> -->
				...
				<tr>
					...
					<td>
			     		<input type="text" name="homePage" class="checkHomepage" value="${subsystem.homePage}" style="width: 700px;" placeholder='请输系统入口' class="input400" required>  
					</td>
				</tr>
				<tr>
					<td style="text-align:right">
				...
				<tr>
					...
					<td>
			     		<input type="text" name="redirectUrl" value="${subsystem.redirectUrl}" placeholder='请输入重定向地址' style="width: 700px;" class="input400" required>  
					</td>
				</tr>
				<tr>
					<td style="text-align:right">
				...
				<tr>
					...
					<td>
						<input id="usingSubAccount" type="radio" name="subaccount" value="1" <c:if test="${subsystem==null || subsystem.subaccount == 1}">checked</c:if>> 
						<label for="usingSubAccount" style="display:inline;">启用</label>
						&nbsp;&nbsp;
						<input id="unUsingSubAccount" type="radio" name="subaccount" value="0"<c:if test="${subsystem!=null && subsystem.subaccount == 0}">checked</c:if>> 
						<label for="unUsingSubAccount"style="display:inline;">不启用</label>
					</td>
				</tr>
				<tr>
					<td style="text-align:right">
					...
					<td>
			     		<input type="text" name="authcUrl" value="${subsystem.authcUrl}" placeholder='请输入子系统授权地址' style="width: 700px;" class="input400" required>  
					</td>
				</tr>
			</table>
