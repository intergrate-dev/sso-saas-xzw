...
<html>
		...
		<script type="text/javascript">
			...
			function deleteSubsystem(userId){
				if(confirm("删除子系统将不可恢复，您确定执行此操作？")){
					location.href= "${ctx}/admin/app/" + userId + "/delete";
				}
			}
		</script>
		<style type="text/css">
	...
	<body>
				...
				<tbody>
		            <c:forEach items="${subsystemList}" var="subsystem" varStatus="i">
			            <tr title="${subsystem.description }">
			                <td>
			                	<a href="${ctx }/admin/app/initUpdate?id=${subsystem.id }">
			                		${subsystem.code }  
			                	</a>		
			                </td>
			                <td>
			                ...
			                <td>
			                	<a href="${ctx }/admin/app/initUpdate?id=${subsystem.id }">
			                		${(subsystem.domain == null || subsystem.domain =='') ? "无": subsystem.domain}
			                	</a>		
			                </td>
			                <td>
			                	<a href="${ctx }/admin/app/initUpdate?id=${subsystem.id }">
			                ...
			                <td>
			                	<a href="${ctx }/admin/app/initUpdate?id=${subsystem.id }">
			                		${(subsystem.loginUrl == null || subsystem.loginUrl =='') ? "无": subsystem.loginUrl}
			                	</a>		
			                </td>
		...
		<div class="offset10">
			<a href="${ctx }/admin/app/initUpdate">
